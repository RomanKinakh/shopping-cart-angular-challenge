...
export class CartFacade {
  ...
  errors = this.store.pipe(select(CartQuery.getError));
  allItems$ = this.store.pipe(select(CartQuery.getAllItems));
  getSelected$ = this.store.pipe(select(CartQuery.getSelected));
  ...
  selectItemId(id: number): void {
    this.store.dispatch(CartActions.setItemId({ id }));
  }
