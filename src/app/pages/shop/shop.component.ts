...
export class ShopComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.activatedRoute.queryParams.pipe(distinctUntilChanged(), takeUntil(this.destroy$)).subscribe((queryParams) => {
  ...
  private getProducts(params: FiltersEntity): void {
    ...
    this.shopService.getData(params).pipe(takeUntil(this.destroy$), finalize(() => this.subjectService.setShowSpinnerValue(false))).subscribe(({ data, total, page }) => {
      this.products = data;
      this.total = total;
      this.page = page;
    })
